...
<body>
	...
	layui.use(['jquery', 'laydate', 'form', 'table'], function(){
		...
		table.render({
		    elem: '#tab'
			,url: 'deadlineAction_findDeadlineList2'
			,cellMinWidth: 80			// 全局定义常规单元格的最小宽度，layui 2.2.1 新增
		    ,cols: [[
		    	   {type:'numbers', title: '序号'}
			      ,{field:'docTheme', title: '主题', templet: '<div>{{d.placeFile.feedback.docTheme}}</div>'}
			      ,{field:'docCreateDate', title: '创建日期', templet: '<div>{{d.placeFile.feedback.docCreateDate}}</div>'}
			      ,{field:'docFindDate', title: '发现日期', templet: '<div>{{d.placeFile.feedback.docFindDate}}</div>'}
			      ,{field:'docFindAddr', title: '发现地点', templet: '<div>{{d.placeFile.feedback.docFindAddr}}</div>'}
		...
		table.on('tool(tabFilter)', function(obj){
			...
			if(layEvent == "deadline") {
				location.href = "deadlineAction_findDeadlineById?deadline.deaId=" + data.deaId;
			}
		});
	...
	<script type="text/html" id="toobarTab">
		<a class="layui-btn layui-btn-xs" lay-event="deadline">考核通报</a>
	</script>
