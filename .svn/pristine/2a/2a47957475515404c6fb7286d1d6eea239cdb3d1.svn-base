...
<body>
	...
	layui.use(['jquery', 'laydate', 'form', 'table'], function(){
		...
		table.render({
		    elem: '#tab'
			,url: 'replyDeadlineAction_findReplyOnDeadline'
		...
		table.on('tool(tabFilter)', function(obj){
			...
			if(layEvent == "dea") {
				location.href = "${pageContext.request.contextPath}/pages/replyDeadline.jsp?repId=" + data.repId + "&deaId=" + data.deadline.deaId;
			} 
		});
	...
	<script type="text/html" id="toobarTab">
		<a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="dea">确认申请</a>
		<a class="layui-btn layui-btn-xs" lay-event="close">确认关闭</a>
	</script>
