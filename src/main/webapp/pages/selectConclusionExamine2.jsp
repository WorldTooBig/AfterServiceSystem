...
<body>
	...
	layui.use(['jquery', 'laydate', 'form', 'table'], function(){
		...
		table.render({
		    elem: '#tab'
			,url: 'conclusionAction_findConclusion2'
			,cellMinWidth: 80 //全局定义常规单元格的最小宽度，layui 2.2.1 新增
		    ,cols: [[
		    	   {type:'numbers', title: '序号'}
			      ,{field:'docTheme', title: '主题', templet: '<div>{{d.replyList[0].notified.placeFile.feedback.docTheme}}</div>'}
			      ,{field:'docCreateDate', title: '创建日期', templet: '<div>{{d.replyList[0].notified.placeFile.feedback.docCreateDate}}</div>'}
			      ,{field:'docFindDate', title: '发现日期', templet: '<div>{{d.replyList[0].notified.placeFile.feedback.docFindDate}}</div>'}
			      ,{field:'docFindAddr', title: '发现地点', templet: '<div>{{d.replyList[0].notified.placeFile.feedback.docFindAddr}}</div>'}
			      ,{field:'repId', title: '答复人', templet: '<div>{{d.conId}}</div>'}
		...
		table.on('tool(tabFilter)', function(obj){
			...
			if(layEvent == "conclusion") {
				location.href = "conclusionAction_findConclusionById?conclusion.conId=" + data.conId;
			}
		});
