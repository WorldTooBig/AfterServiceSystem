...
<body>
	...
	layui.use(['jquery', 'laydate', 'form', 'table'], function(){
		...
		table.render({
		    elem: '#tab'
			,url: 'placeFileAction_findPlaceFileListOnDeadline'
			,cellMinWidth: 80 //全局定义常规单元格的最小宽度
		    ,cols: [[
		    	   {type:'numbers', title: '序号'}
			      ,{field:'docTheme', title: '主题', templet: '<div>{{d.feedback.docTheme}}</div>'}
		...
		table.on('tool(tabFilter)', function(obj){
			...
			if(layEvent == "deadline") {
				location.href = "placeFileAction_findPlaceFileByIdOnDeadline?placeFile.plaId=" + data.plaId;
			}
		});
	...
	<script type="text/html" id="toobarTab">
		<a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="deadline">处理</a>
	</script>
