...
<body>
	...
	layui.use(['jquery', 'laydate', 'form', 'table'], function(){
		...
		table.render({
		    elem: '#tab'
			,url: 'placeFileAction_findPlaceFileList'
		    ...
		    ,cols: [[
			      ...
			      ,{field:'docFindAddr', title: '发现地点',edit: 'text'}
		...
		table.on('tool(tabFilter)', function(obj){
			...
			if(layEvent == "notifide") {
				location.href = "${pageContext.request.contextPath}/pages/notified.jsp?plaId=" + data.plaId;
			} else if(layEvent == "deadline") {
				location.href = "${pageContext.request.contextPath}/pages/deadline.jsp?plaId=" + data.plaId;
			}
		});
	...
	<script type="text/html" id="toobarTab">
		...
		<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="deadline">限期整改</a>
	</script>
